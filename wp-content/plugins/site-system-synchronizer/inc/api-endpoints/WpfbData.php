<?php
/** Page Builder Framework Endpoint */
...
class WpfbData extends BaseEndpoint {

	public static $route = 'wpbf/data';
		...
		if($this->sync_global){
			switch($action){
				case "UpdateOrInsertOptions":
					if($this->options['sync_items']['wpbf_settings']){
						$res = $this->UpdateOrInsertOptions($data);
					}
					break;
				case "UpdateOrInsertPosts":
					if($this->options['sync_items']['wpbf_posts']){
