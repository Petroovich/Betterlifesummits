...
class Helper {
	...
	public static function initialise(){
		...
		if(!is_dir(S3_DOWNLOAD_DIR)){
			@mkdir(S3_DOWNLOAD_DIR, 0777);
		}
		
		if(!is_dir(S3_LOG_DIR)){
			@mkdir(S3_LOG_DIR, 0777);
		}
		
		
		if(!file_exists(S3_LOG_DIR.'/'.self::$log_file)){
			self::_log('#START');
			@chmod(S3_LOG_DIR.'/'.self::$log_file, 0666);
		}
	...
	public static function _log($content){
		...
		file_put_contents(S3_LOG_DIR.'/'.self::$log_file, $content, FILE_APPEND);
	}

	public static function clearDownloadFolder($file_type = 'zip', $limit_check = 20){
		$files = array_diff(scandir(S3_DOWNLOAD_DIR), ['.', '..']);

		if(count($files) < $limit_check) return;

		@chmod(S3_DOWNLOAD_DIR, 0777);

		foreach($files as $file){
			$ext = self::get_file_ext($file);
			if($ext == $file_type){
				@unlink(S3_DOWNLOAD_DIR.DIRECTORY_SEPARATOR.$file);
			}
		}
	}
